...
//  ********** Library CodeMirrorExample **************
// ********** Code for CodeMirrorExample **************
function CodeMirrorExample() {

}
CodeMirrorExample.prototype.run = function() {
  var codeMirror = new CodeMirrorImpl();
  html_get$document().get$window().get$on().get$resize().add((function (_) {
    return codeMirror.viewResizeHandler(html_get$document());
  })
  , false);
  var root = codeMirror.getMirror();
  html_get$document().get$body().get$nodes().add(root);
  codeMirror.viewResizeHandler(html_get$document());
}
...
function CodeMirrorImpl() {
  ...
  var buttonStr = "<button id=\"submitButton\" style=\"visibility: hidden;\" value=\"Submit\">Submit</button>";
  var viewStr = "<div id=\"view\" class=\"view\"><div>";
  this.console = ElementWrappingImplementation.ElementWrappingImplementation$html$factory("<div><p id='results'></p></div>");
...
(function(){
  ...
  var v15/*Node*/ = [v4/*CharacterData*/,v5/*Document*/,v6/*Element*/,'Node|Attr|DocumentFragment|DocumentType|Entity|EntityReference|Notation|ProcessingInstruction'].join('|');
  var v16/*SVGStylable*/ = 'SVGStylable|SVGFilterPrimitiveStandardAttributes';
